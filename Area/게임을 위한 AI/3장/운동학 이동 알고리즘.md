...
### **자료 구조 및 인터페이스**
3장 초기에서 설명했던 Static 데이터를 사용하고 출력 값으로 KinematicsSteerinOutput 구조를 사용했고 KinematicSteeringOutput은 다음과 같다
```의사코드
class KinematicSteeringOutput:
	valocity: Vector
	rotation: float
...
위 알고리즘에서 캐릭터의 방향은 간단하게 현재 움직이는 방햐으로 설정하고 회전(rotation)은 전혀 사용하지 않았으며 newOrientation을 주석 처리해 원하는 대로 방위 값을 설정할 수 있다.
